class ConfigTemplate < ActiveRecord::Base
  ...
  def attrs
    ...
    attrs.nil? ? nil : YAML.load(attrs[1])
  rescue SyntaxError
    nil
  end

  def supports?(os, major, minor)
    return false unless os_hash.key? os

    if os_hash[os].blank?
      # This template supports all versions of this OS
      true
  ...
  def set_os_hash
    return unless attrs["oses"].is_a? Array
    # Get a hash like this:
    #   {"CentOS"=>{4=>[], 5=>[], 6=>[], 7=>[]}, "Fedora"=>{16=>[], 17=>[], 18=>[], 19=>[], 20=>[]}}
    # Or this:
    #   {"Debian"=>{6=>[0], 7=>[0]}, "Ubuntu"=>{10=>[4], 12=>[4], 13=>[4]}}
